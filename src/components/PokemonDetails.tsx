...
export const PokemonDetails = ({ pokemon }: { pokemon: Pokemon }) => {
  ...
  return (
    <>
              ...
              {pokemon.stats.map((s) => (
                <div key={s.stat.name}>
                  ...
                  <div className="w-full bg-white bg-opacity-20 h-3 rounded overflow-hidden">
                    <div className="h-full bg-white bg-opacity-70 rounded transition-all duration-1000" style={{ width: `${s.base_stat / 2}%` }} />
                  </div>
                </div>
              ))}
            </div>
      ...
      {showDrawer && <EvolutionDrawer evolutions={evolutions} gradient={gradient} onClose={() => setShowDrawer(false)} />}
    </>
  );
};
