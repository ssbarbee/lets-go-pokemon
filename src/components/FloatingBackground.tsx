    ...
    return (
        <div className="absolute inset-0 overflow-hidden z-0">
            {floatingPokemons.map((poke, idx) => (
                <Link href={`/pokemon/${poke.id}`} key={idx}>
                    <div
                        key={idx}
                    ...
                    >
                        <div
                            className="relative flex items-center justify-center w-20 h-20 rounded-full bg-white bg-opacity-30 backdrop-blur-md group-hover:scale-125 transition">
                            <Image
                                src={poke.src}
                                alt={poke.label}
                                width={60}
                        ...
                        {/* Label on Hover */}
                        <div
                            className="absolute top-full left-1/2 -translate-x-1/2 mt-2 text-xs text-white bg-black bg-opacity-60 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition">
                            {poke.label}
                        </div>
                    </div>
                </Link>
